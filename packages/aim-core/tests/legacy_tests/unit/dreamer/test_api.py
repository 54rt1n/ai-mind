...
async def test_start_pipeline_success(mock_config, mock_state_store, mock_scheduler, simple_scenario):
    ...
    with patch('aim_legacy.dreamer.server.api.load_scenario') as mock_load_scenario, \
         patch('aim_legacy.dreamer.server.api.ConversationModel') as mock_cvm_class, \
         patch('aim_legacy.dreamer.server.api.Roster') as mock_roster_class, \
         patch('aim_legacy.dreamer.server.api.LanguageModelV2') as mock_model_class, \
         patch('aim_legacy.dreamer.server.api.run_seed_actions') as mock_run_seed:
...
async def test_list_pipelines_with_results(mock_state_store, mock_pipeline_state, simple_scenario):
    ...
    with patch('aim_legacy.dreamer.server.api.load_scenario') as mock_load_scenario, \
         patch('aim_legacy.dreamer.server.api.get_status') as mock_get_status:
