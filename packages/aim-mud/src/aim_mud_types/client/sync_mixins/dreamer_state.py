# aim-mud-types/client/sync_mixins/dreamer_state.py
# AI-Mind (C) 2025 by Martin Bukowski is licensed under CC BY-NC-SA 4.0
"""Sync DreamerState-specific operations."""
...
class SyncDreamerStateMixin:
    """Sync DreamerState-specific Redis operations.

    Provides CRUD operations for automatic dreaming configuration.
    """

    def get_dreamer_state(
        ...
        return self._get_hash(DreamerState, key)

    def update_dreamer_state_fields(
        self: "BaseSyncRedisMUDClient",
        agent_id: str,
        **fields
    ) -> bool:
        ...
        Example:
            client.update_dreamer_state_fields(
                "andi",
                enabled=True,
    ...
    def has_running_dream(
        ...
        state = self._get_hash(DreamingState, key)
    ...
    def get_dreaming_state(
        self: "BaseSyncRedisMUDClient",
        agent_id: str
    ) -> Optional[DreamingState]:
        """Fetch active dreaming state for an agent."""
        key = RedisKeys.agent_dreaming_state(agent_id)
        return self._get_hash(DreamingState, key)

    def abort_running_dream(
        self: "BaseSyncRedisMUDClient",
        agent_id: str,
        ...
        """
        state = self.get_dreaming_state(agent_id)
        if state is None:
            return False
        ...
        self.redis.lpush(history_key, state.model_dump_json())
