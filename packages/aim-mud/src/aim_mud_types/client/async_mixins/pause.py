...
class PauseMixin:
    ...
    async def is_paused(self: "BaseAsyncRedisMUDClient", pause_key: str) -> bool:
        """Return True if pause flag is set for key."""
        value = await self.redis.get(pause_key)
        return _is_pause_value(value)

    async def is_agent_paused(self: "BaseAsyncRedisMUDClient", agent_id: str) -> bool:
        """Return True if agent pause flag is set."""
        return await self.is_paused(RedisKeys.agent_pause(agent_id))

    async def is_mediator_paused(self: "BaseAsyncRedisMUDClient") -> bool:
