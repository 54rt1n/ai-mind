...
class TurnRequestMixin:
    ...
    async def get_turn_request(self: "BaseAsyncRedisMUDClient", agent_id: str) -> Optional[MUDTurnRequest]:
    ...
    async def initialize_turn_request(
        self: "BaseAsyncRedisMUDClient",
        agent_id: str,
        *,
        ...
        **metadata_kwargs,
        ...
        existing = await self.get_turn_request(agent_id)
        if existing:
        ...
        created = await self.create_turn_request(agent_id, turn_request)
        if created:
    ...
    async def assign_turn_request(
        ...
        **metadata_kwargs,
        ...
        paused = await self.redis.get(pause_key)
        if paused in (b"1", "1"):
        ...
        if not skip_availability_check or expected_turn_id is None:
            current = await self.get_turn_request(agent_id)
            if not current:
        ...
        success = await self.update_turn_request(agent_id, turn_request, expected_turn_id=expected_turn_id)
        if not success:
            return False, None, "cas_failed"

        return True, turn_request, "ok"

    async def transition_turn_request_and_update(
        self: "BaseAsyncRedisMUDClient",
        agent_id: str,
        turn_request: MUDTurnRequest,
        expected_turn_id: str,
        *,
        ...
        return await self.update_turn_request(agent_id, turn_request, expected_turn_id=expected_turn_id)

    async def transition_turn_request_to_ready(
        self: "BaseAsyncRedisMUDClient",
        agent_id: str,
        turn_request: MUDTurnRequest,
        expected_turn_id: str,
